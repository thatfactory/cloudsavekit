...
`sendNow()` reloads that ledger before sending. This makes the host the source of truth if CKSyncEngine discarded a semantic failure or if a previously persisted checkpoint still contains a change the host has since acknowledged. If the host commits enqueues or acknowledgements while a ledger read is suspended, CloudSaveKit replays those mutations in their original order after reconciling the returned snapshot.
